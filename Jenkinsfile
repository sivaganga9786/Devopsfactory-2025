...
pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        git branch: 'feature/VPC-Multi-Instances', credentialsId: 'github_cred', url: 'https://github.com/sivaganga9786/Devopsfactory-2025.git'
      }
    }
   ...
   stage('Stage IV: SAST') {
      steps { 
        ...
        withSonarQubeEnv('mysonarqube') {
            sh 'mvn sonar:sonar -Dsonar.coverage.jacoco.xmlReportPaths=target/site/jacoco/jacoco.xml -Dsonar.dependencyCheck.jsonReportPath=target/dependency-check-report.json -Dsonar.dependencyCheck.htmlReportPath=target/dependency-check-report.html -Dsonar.projectName=Terraform_ci'
       }
      }
    }
